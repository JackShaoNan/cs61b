...
public class Maze {
    ...
    public Maze(int horizontalSize, int verticalSize) {
        ...
        //2.
        int[] xWall = new int[horiz * (vert - 1)];
        int[] yWall = new int[(horiz - 1) * vert];
        for(int k = 0; k < xWall.length; ++k) {
            xWall[k] = k;
        }
        for(int k = 0; k < yWall.length; ++k) {
            yWall[k] = k;
        }
        int w = xWall.length - 1;
        for(; w >= 1; w--) {
            int randInt = randInt(w);
            int temp = xWall[randInt];
            xWall[randInt] = xWall[w];
            xWall[w] = temp;
        }
        for(w = yWall.length - 1; w > 0; w--) {
            int randInt = randInt(w);
            int temp = yWall[randInt];
            yWall[randInt] = yWall[w];
            yWall[w] = temp;
        }
        //3.
        for(w = 0; w < xWall.length; ++w) {
            //determine the upper and bottom item of wall
            int[] upper = {xWall[w] % horiz, (xWall[w] - (xWall[w] % horiz)) / horiz};
            int[] bottom = {upper[0], upper[1] + 1};
            //check if these two items are in the same set
            if(ds.find(upper[0] + upper[1] * horiz) != ds.find(bottom[0] + bottom[1] * horiz)) {
                //set the wth xWall to be false
                hWalls[upper[0]][upper[1]] = false;
                //union two sets
                ds.union(upper[0] + upper[1] * horiz, bottom[0] + bottom[1] * horiz);
            }
        }
        for(w = 0; w < yWall.length; ++w) {
            //determine the left and right item of wall
            int[] left = {yWall[w] % (horiz - 1), (yWall[w] - (yWall[w] % (horiz - 1))) / (horiz - 1)};
            int[] right = {left[0] + 1, left[1]};
            //check if these two items are in the same set
            if(ds.find(left[0] + left[1] * horiz) != ds.find(right[0] + right[1] * horiz)) {
                //set the wth yWall to be false
                vWalls[left[0]][left[1]] = false;
                //union two sets
                ds.union(left[0] + left[1] * horiz, right[0] + right[1] * horiz);
            }
        }
